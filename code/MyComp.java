    ...
    public static void compress() throws IOException {
        ...
        try {
            ...
            while (true) {
                 ...
                 * so far which is stored in P. The value of this string is converted in
                 * binary. This binary number is then padded to make it 12 bits long (for
                 * standardization and avoing overflow or underflow caused using 8 bits). This
                 * is then converted into bytes and stored.
                 * 
        ...
        /*
         * If isLeft is true, we store the current data in BP to buffer[0] and buffer[1].
         * Then these buffers are written in the output file. If isLeft is false, we
         * already have data in the first and half of seccond byte of the buffer. Hence,
         * we store the current value of BP and write all the 3 bytes to the outputFile.
         * 
         * When the file input is complete, the while loop will still execute due to the
         * condition. This ensures that the file is read completely but it might throw
         * an error if there is no input left in the inputFile. So, when an error is
         * thrown, we store the remaining contents of the buffer.
         */
